...
namespace api.Services
{
    ...
    internal static class FieldDescriptionCatalog
    {
        ...
        [
            ...
            // Fonds euros
            new("EuroFunds", "fund.code", "Code technique du support fonds euros sélectionné. Il permet de rattacher la configuration et les exercices au bon support du référentiel des valeurs."),
            new("EuroFunds", "fund.label", "Libellé lisible du fonds euros. C'est le nom utilisé par les gestionnaires pour identifier le support dans l'écran de paramétrage."),
            new("EuroFunds", "fund.currency", "Devise de référence du fonds euros. Les montants de réserve, d'encours et de participation aux bénéfices sont exprimés dans cette devise."),
            new("EuroFunds", "configuration.accrualMethod", "Convention de calcul des intérêts du fonds euros. Elle détermine la manière de proratiser la rémunération entre deux dates, par exemple en jours réels ou par quinzaine."),
            new("EuroFunds", "configuration.annualCreditMonth", "Mois auquel la participation aux bénéfices annuelle est créditée sur les contrats. Cette donnée sert à positionner l'opération comptable de revalorisation."),
            new("EuroFunds", "configuration.annualCreditDay", "Jour du mois auquel la participation aux bénéfices annuelle est créditée. Il complète le mois de crédit pour fixer la date d'effet de l'attribution annuelle."),
            new("EuroFunds", "configuration.provisionalRateMethod", "Méthode utilisée pour déterminer un taux provisoire lorsque le taux final de l'exercice n'est pas encore connu. Elle encadre les calculs de rachat, décès ou simulation intermédiaire."),
            new("EuroFunds", "configuration.provisionalRatePercentage", "Pourcentage du TME retenu lorsque le taux provisoire est indexé sur le TME. Par exemple, 80 signifie que le taux provisoire vaut 80 % du TME de référence."),
            new("EuroFunds", "configuration.fixedProvisionalRate", "Taux provisoire fixe appliqué lorsque la méthode de taux provisoire est définie en valeur fixe. Il sert de taux d'attente avant fixation du taux final."),
            new("EuroFunds", "configuration.previousFinalRatePercentage", "Part du dernier taux final connu à reprendre pour calculer un taux provisoire ou de sortie anticipée. Cette valeur permet de prolonger prudemment une référence historique."),
            new("EuroFunds", "configuration.earlyExitRateMethod", "Règle de rémunération appliquée aux capitaux sortis avant la clôture annuelle. Elle précise si la sortie utilise le taux final connu, un taux provisoire, un taux garanti ou une autre référence."),
            new("EuroFunds", "configuration.lotConsumptionMethod", "Méthode de consommation des lots de fonds euros lors d'une sortie. Elle indique si les lots les plus anciens, les plus récents, les lots bonifiés ou une répartition prorata sont utilisés en priorité."),
            new("EuroFunds", "configuration.rateNature", "Nature du taux paramétré pour la configuration du fonds euros. Elle indique si le taux est net de frais de gestion ou brut avant prélèvement de ces frais."),
            new("EuroFunds", "configuration.managementFeeTreatment", "Mode de traitement des frais de gestion du fonds euros. Il précise si les frais sont déjà inclus dans le taux servi ou s'ils doivent donner lieu à une opération de frais distincte."),
            new("EuroFunds", "configuration.minimumGuaranteedRate", "Taux minimum contractuellement garanti lorsqu'il existe. Il constitue un plancher de rémunération à respecter indépendamment du rendement final."),
            new("EuroFunds", "configuration.rateFloor", "Plancher opérationnel appliqué au taux calculé. Il empêche la rémunération servie de descendre sous une limite définie par la règle métier."),
            new("EuroFunds", "configuration.rateCap", "Plafond opérationnel appliqué au taux calculé. Il limite la rémunération servie lorsqu'une règle contractuelle ou de gestion impose un maximum."),
            new("EuroFunds", "financialYear.year", "Année de l'exercice de participation aux bénéfices. Elle identifie la période annuelle pour laquelle les réserves, rendements et taux servis sont saisis ou calculés."),
            new("EuroFunds", "financialYear.tmeRate", "Taux moyen des emprunts d'État utilisé comme référence de marché pour l'exercice. Il peut servir à calculer un taux provisoire ou à contrôler la cohérence de la rémunération."),
            new("EuroFunds", "financialYear.assetYield", "Rendement financier brut ou constaté des actifs adossés au fonds euros sur l'exercice. Il donne le niveau de performance disponible avant décision de distribution et de réserve."),
            new("EuroFunds", "financialYear.openingPpbReserve", "Montant de provision pour participation aux bénéfices disponible au début de l'exercice. Cette réserve peut compléter ou lisser la rémunération servie aux contrats."),
            new("EuroFunds", "financialYear.ppbAllocation", "Dotation de l'exercice à la provision pour participation aux bénéfices. Elle correspond à la part du résultat mise en réserve au lieu d'être immédiatement distribuée."),
            new("EuroFunds", "financialYear.ppbRelease", "Reprise de provision pour participation aux bénéfices utilisée sur l'exercice. Elle augmente la capacité de distribution aux assurés à partir d'une réserve constituée antérieurement."),
            new("EuroFunds", "financialYear.closingPpbReserve", "Montant de provision pour participation aux bénéfices restant en fin d'exercice. Il est obtenu après dotations, reprises et décisions de distribution."),
            new("EuroFunds", "financialYear.finalServedRate", "Taux final effectivement servi aux contrats pour l'exercice. C'est la référence de rémunération annuelle une fois l'exercice arrêté ou finalisé."),
            new("EuroFunds", "financialYear.rateNature", "Nature du taux final saisi pour l'exercice. Elle indique si le taux final est net de frais de gestion ou brut avant frais."),
            new("EuroFunds", "financialYear.status", "Statut de l'exercice annuel. Ouvert signifie saisissable, provisoire signifie exploitable pour simulation, finalisé signifie arrêté pour application définitive."),
            new("EuroFunds", "referenceRate.rateDate", "Date de constatation du TME historisé. Elle permet de conserver la référence de marché utilisée pour les calculs ou contrôles de l'exercice."),
            new("EuroFunds", "referenceRate.rateValue", "Valeur du TME historisé à la date indiquée. Cette donnée alimente les méthodes de taux provisoire ou de sortie fondées sur le TME."),
            new("EuroFunds", "referenceRate.source", "Origine de la donnée TME historisée, par exemple back-office, import ou publication externe. Elle facilite l'audit et la justification du taux de référence."),
            new("EuroFunds", "preview.contractCount", "Nombre de contrats concernés par la simulation ou la finalisation de participation aux bénéfices. Il permet de mesurer le périmètre réellement traité."),
            new("EuroFunds", "preview.totalBookValue", "Encours acquis total retenu pour le calcul de participation aux bénéfices. Il agrège la valeur fonds euros des contrats éligibles sur l'exercice."),
            new("EuroFunds", "preview.totalParticipationBenefit", "Montant total de participation aux bénéfices calculé pour le périmètre simulé. Il correspond à la somme à créditer ou à comptabiliser sur les contrats concernés."),
            new("EuroFunds", "preview.averageParticipationBenefit", "Montant moyen de participation aux bénéfices par contrat traité. Cet indicateur aide à contrôler rapidement l'ordre de grandeur de la distribution."),
            new("EuroFunds", "preview.appliedRate", "Taux effectivement utilisé par la simulation. Il peut provenir du taux final si connu ou d'une règle de taux indicatif lorsque l'exercice n'est pas finalisé."),
            new("EuroFunds", "preview.usesFinalRate", "Indique si la simulation repose sur le taux final de l'exercice. Lorsque ce n'est pas le cas, le résultat doit être lu comme indicatif."),
            new("EuroFunds", "preview.contractNumber", "Numéro du contrat affiché dans le résultat de simulation. Il sert à identifier la ligne de calcul et à rapprocher le résultat du dossier client."),
            new("EuroFunds", "preview.bookValue", "Encours fonds euros du contrat retenu dans la simulation. Il représente la base de valeur sur laquelle la participation aux bénéfices est calculée."),
            new("EuroFunds", "preview.weightedExposure", "Exposition pondérée du contrat sur l'exercice. Elle tient compte des dates d'entrée, de sortie ou de mouvements pour proratiser la rémunération."),
            new("EuroFunds", "preview.participationBenefit", "Participation aux bénéfices calculée pour le contrat. C'est le montant estimé ou définitif à attribuer au contrat au titre de l'exercice."),
            new("EuroFunds", "preview.details", "Nombre de segments de calcul utilisés pour le contrat. Plusieurs segments peuvent exister lorsqu'il y a des mouvements, des lots ou des périodes de taux différentes."),
            new("EuroFunds", "preview.errors", "Alertes ou anomalies détectées pendant la simulation. Elles signalent les contrats ou règles qui doivent être contrôlés avant finalisation."),
