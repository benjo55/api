...
{
    public sealed class AssetAllocationReportDocumentDataProvider : IDocumentDataProvider
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var documentRequest = new GenerateAssetAllocationReportRequestDto
            {
                ContractId = contractId,
                FileName = ReadStringParameter(request, "fileName")
                    ?? $"allocation-actifs-{contractId.ToString(CultureInfo.InvariantCulture)}",
                LogoBase64 = ReadStringParameter(request, "logoBase64"),
                LogoUrl = ReadStringParameter(request, "logoUrl")
            };

            return Task.FromResult<object>(new AssetAllocationReportDocumentModel(documentRequest));
        }
