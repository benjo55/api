...
{
    public sealed class OperationsHistoryPdfRenderer : IDocumentRenderer
    {
        private readonly IPdfBusinessDocumentService _pdfBusinessDocumentService;

        public OperationsHistoryPdfRenderer(IPdfBusinessDocumentService pdfBusinessDocumentService)
        {
        ...
        {
            var document = (OperationsHistoryDocumentModel)model;
            var result = await _pdfBusinessDocumentService.GenerateOperationsHistoryAsync(
                document.Request,
                cancellationToken);
